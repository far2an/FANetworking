...
class DownloadTests: XCTestCase {
    ...
    func testImageDownloadWithWeirdCharacters() {
        let networking = Networking(baseURL: "https://sample-videos.com")
        let path = "/img/Sample-jpg-image-50kb.jpg"
        ...
        networking.downloadImage(path) { result in
            switch result {
            case let .success(response):
                let pigImage = Image.find(named: "sample.jpg", inBundle: Bundle(for: DownloadTests.self))
                let pigImageData = pigImage.pngData()
                let imageData = response.image.pngData()
                XCTAssertEqual(pigImageData, imageData)
            case .failure:
